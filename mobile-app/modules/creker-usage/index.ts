...
export interface AppInfo {
  ...
  /**
   * Обрабатывает ли пакет кнопку «домой», то есть является ли он оболочкой телефона.
   *
   * Домашний экран — не приложение, которым пользуются: он мелькает между всем остальным.
   * Отсюда и его числа: часы времени и тысячи «открытий», в которых нет ни одного
   * намеренного.
   */
  isHome: boolean;
}
